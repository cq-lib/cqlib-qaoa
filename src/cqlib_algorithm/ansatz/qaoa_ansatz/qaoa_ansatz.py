...
def build_cost_layer(circ: Circuit, h: Dict[int, float], J: Dict[Edge, float], gamma: float):
...
def build_qaoa_circuit(
    ...
    if betas is None:
        betas  = [0.2] * reps
    assert len(gammas) == reps and len(betas) == reps, "betas/gammas must be length `reps`"
    ...
    if initial_state is None or initial_state == "plus":
            ...
            if hasattr(init_circ, "operations"):
                for op in init_circ.operations:
                    try:
                        ...
                        raise RuntimeError("Failed to merge `initial_state` circuit into main circuit.")
            else:
                raise RuntimeError("Initial_state is a Circuit but lacks a usable merge/iteration API.")
        ...
        init_info = f"callable init_fn {getattr(initial_state, '__name__', str(initial_state))}" 
    else:
    ...
    if mixer_operator is None or mixer_operator == "x":
        ...
        mix_info = f"custom mixer {getattr(mixer_operator, '__name__', str(mixer_operator))}" 
    else:
        raise ValueError("`mixer_operator` must be None/'x'/'xy'/Callable(circ, qubits, beta)")
    ...
    _qaoa_meta = {
        ...
        "betas": betas,
        "gammas": gammas,
        "mixer": ("x" if (mixer_operator is None or mixer_operator == "x")
                  else "xy" if mixer_operator == "xy"
                  else getattr(mixer_operator, "__name__", str(mixer_operator))),
        "initial_state": (
            "|+>^n" if (initial_state is None or initial_state == "plus")
