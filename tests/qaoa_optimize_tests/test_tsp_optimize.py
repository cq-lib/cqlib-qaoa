...
def main():
    ...
    distance_matrix = np.array(
        [[0, 26, 30, 19], [26, 0, 4, 17], [30, 4, 0, 11], [19, 17, 11, 0]]
    )
    ...
    opt_cfg = OptimizerOptions(
        name="spsa", options={"maxiter": 5, "a": 0.2, "c": 0.2}
    )
    # COBYLA
    ...
    res.plot_tsp_solution(
        distance_matrix=tsp.distance_matrix, title="TSP Solution(QAOA)"
    )
