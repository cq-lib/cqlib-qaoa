...
def _assignment_to_routes(X: List[List[List[int]]]) -> List[List[int]]:
    ...
    assigned_slot: List[List[Optional[int]]] = [[None for _ in range(K)] for _ in range(P)]
...
def plot_vrp_solution(
    ...
    *,
    ...
    _, routes, _ = decode_from_platform_result(
        result, n=n, vehicle_count=vehicle_count, positions_per_vehicle=positions_per_vehicle, ising=ising
    )
