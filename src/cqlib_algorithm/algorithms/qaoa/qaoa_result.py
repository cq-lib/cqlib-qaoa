...
class QAOAResult:
    ...
    def plot_probability(self, *, title: str = "QAOA Result Probability", topk: int = 20, show: bool = True):
        ...
        if not self.result_raw or "probability" not in self.result_raw:
            raise ValueError("No available probability distribution in `result_raw['probability']`.")
    ...
    def plot_maxcut_solution(
        ...
        *,
        ...
        part = plot_maxcut_solution(
            n=n, weights=weights, result=self.result_raw,
            choose_ones=choose_ones, title=title, show=show, ising=self.ising
        )
        return part
    ...
    def plot_tsp_solution(
        ...
        *,
        ...
        tour = plot_tsp_solution(
            distance_matrix=distance_matrix, result=self.result_raw,
            title=title, show=show, ising=self.ising
        )
        return tour
    ...
    def plot_vrp_solution(
        ...
        *,
        ...
        routes = plot_vrp_solution(
            distance=distance, result=self.result_raw, n=n, vehicle_count=vehicle_count,
            positions_per_vehicle=positions_per_vehicle, depot=depot,
            title=title, show=show, ising=self.ising
        )
        return routes
