...
class COBYLA(Optimizer):
    ...
    def __init__(self, cfg: OptimizerOptions):
        ...
        self.tol: Optional[float] = float(opts["rhoend"]) if "rhoend" in opts else (  
            float(opts["tol"]) if "tol" in opts else None                       
        )
        self.catol: Optional[float] = float(opts.get("catol", 2e-4))
        self.f_target: Optional[float] = float(opts.get("f_target", None)) if "f_target" in opts else None 
