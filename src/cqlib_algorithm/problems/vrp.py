    ...
    def __post_init__(self):
        ...
        if self.distance is None or np.size(self.distance) == 0:
            self.distance = self._random_distance(
                self.n, method=self.gen_method, coord_range=self.coord_range,
                dim=self.dim, as_int=self.as_int, round_digits=self.round_digits,
                weight_range=self.weight_range, seed=self.seed,
